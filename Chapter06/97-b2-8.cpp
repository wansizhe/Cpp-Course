...
int main()
{
	...
	int row, line, X, Y, getX[22][22] = { 0 }, bgetx[22][22] = { 0 }, score = 0, target = 10;
	...
	while (1)
	{
		...
		if(uncont)
		{
			showch(hout, 0, 4 * row + 1, 15, 0, ' ', 44);
			gotoxy(hout, 0, 4 * row + 1);
			setcolor(hout, 14, 0);
			cout << "按方向键然后按回车键，选择并标记相邻相同数字";
		}
		else
		{
			showch(hout, 0, 4 * row + 1, 15, 0, ' ', 44);
		...
		if (arrow == 13)
		{
			...
			else if (unmerg)
			{
				...
				cout << "本次得分：" << ds << "   总得分：" << score << "   合成目标：" << target;
				/*下落*/
		...
		/*完成判定*/
		if (the_max(getX, row, line) == target)
		{
			gotoxy(hout, 0, 4 * row + 3);
			setcolor(hout, 15, 12);
			cout << "恭喜你！已合成10，继续！";
			setcolor(hout, 15, 0);
			target++;
	...
	gotoxy(hout, 0, 4 * row + 3);
	setcolor(hout, 15, 12);
	cout << "游戏结束！       总得分：" << score << endl;
	setcolor(hout, 15, 0);
	setcolor(hout, 15, 0);
	gotoxy(hout, 0, 4 * row + 4);
	cout << endl;
	return 0;
}