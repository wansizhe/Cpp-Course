...
void printall(HANDLE hout, int getX[][22], int line, int row)
{
	...
	for (int i = 1; i <=3*row+1; i++)
	{
		gotoxy(hout, 0, i);
		cout << "│";
		gotoxy(hout, 6 * line + 2, i);
		cout << "│";
		Sleep(100);
	}
	gotoxy(hout, 0, 3*row+1);
	cout << "┕";
	Sleep(100);
	for (int i = 1; i <= 3*line; i++)
	{
	...
	for (int i = 1; i <= row; i++)
	{
		for (int j = 1; j <= line; j++)
		{
			gotoxy(hout, 6 * (j-1)+2, 3 * (i-1)+1);
			printone(hout, getX[i][j], 6 * (j-1)+2, 3 * (i-1)+1);
			Sleep(50);
		}
		cout << endl;
	}
...
int main()
{
	...
	system("cls");
	char cmd[80];
	sprintf(cmd, "mode con cols=%d lines=%d", 6*line+4, 4*row);
	system(cmd);
